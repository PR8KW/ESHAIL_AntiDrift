...
# Rev v0.01 for SDR Console V2.3 by Gustavo Schwartz, PR8KW
#
...
COM_beacon = 'COM21'     # CAT-COM-port of the RX listening to the beacon
COM_band = 'COM21'       # CAT-COM-port of the RX listening to the band
...
virtual_audio_device = "CABLE Output (VB-Audio Virtual Cable), Windows DirectSound" 
virtual_audio_sample_rate = 44100   # see VAC Control Panel, when sound device is opened by SDRuno
...
pid.output_limits = (-400, 400)    # max. frequency change in Hz per pid.sample_time
...
serial_beacon = serial.Serial(COM_beacon, baudrate, timeout=1.00) # write blocking, read non-blocking
# serial_band = serial.Serial(COM_band, baudrate, timeout=1.00) # write blocking, read non-blocking
serial_band = serial_beacon
...
def CAT_updateRX1(serial, control_value):
   serial.write(b'FA;')
...
def CAT_updateRX2(serial, control_value):
   serial.write(b'FB;')
   data_string = serial.read(14).decode('ascii')
   FB_regex = r"^FB(\d{11});$"   
   if re.match(FB_regex, data_string) != None:
      frequency = int(re.sub(FB_regex, r"\1", data_string))            
      sleep(0.05)            
      message_string = "FB{:011d};".format(frequency - control_value)   
      serial.write(message_string.encode('ascii'))
      return frequency - control_value
...
with sd.InputStream(channels=1, callback=callback, blocksize=fft_blocksize):
   while True:
      ...
      relative_beacon_frequency = CAT_updateRX2(serial_beacon, control_value)
      relative_band_frqeuncy = CAT_updateRX1(serial_band, control_value)
